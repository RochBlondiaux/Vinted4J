...
@Log4j2
public record ResponseHandler<T extends VintedResponse, R extends VintedRequest<T>>(R request) {
    ...
    public <U> U parseResponse(String json, Class<U> type) throws JsonProcessingException {
        log.debug("{}{} parsing response : {}", request.apiPath(), request.endpoint(), json);
