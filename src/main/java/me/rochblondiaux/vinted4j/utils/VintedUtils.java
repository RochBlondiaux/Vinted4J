...
public class VintedUtils {
     ...
     * @return
     */
    private static String digest(String codec, String source) {
        try {
     ...
     * @return
     */
    private static String hexlate(byte[] bytes, int initialCount) {
        if (bytes == null) {
            return "";
        }
    ...
    public static Optional<String> getCookieValue(CookieJar jar, String key) {
        return jar.loadForRequest(HttpUrl.get(VintedConstants.BASE_API_URL)).stream()
