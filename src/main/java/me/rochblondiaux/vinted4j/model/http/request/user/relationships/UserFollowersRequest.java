...
public record UserFollowersRequest(long userId, int page, int perPage) implements GETRequest<UsersListResponse> {

    @Override
    public String endpoint(VintedAPI client) {
        return "users/%d/followers?page=%d&per_page=%d".formatted(userId, page, perPage);
    }
